...
const chat = message.guild.channels.cache.find(r => r.id === (Settings.Channels.GeneralChat))
  ...
  let isim = args[1]
  let yaş = args[2]
  let uyarıembed = new MessageEmbed().setColor(Settings.Colors.Red).setFooter(Settings.BotSettings.botStatus).setTimestamp()
...
.setTimestamp()
.setFooter(Settings.BotSettings.botStatus)
.setColor(Settings.Colors.Blue)
chat.send(chatembed)
}
exports.conf = {
